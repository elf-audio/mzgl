...
struct NVGcontext {
	std::function<void(float *dx, float *dy, float sx, float sy)> xformFn;
	std::function<void(float *m3x3)> xformMultFn;
	NVGparams params;
	float* commands;
	int ccommands;
	int ncommands;
...
void nvgSetTranformFunction(NVGcontext *ctx, std::function<void(float *dx, float *dy, float sx, float sy)> fn) {
	ctx->xformFn = fn;
}

void nvgSetTranformMultFunction(NVGcontext *ctx, std::function<void(float*)> fn) {
	ctx->xformMultFn = fn;
}
...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
	...
	fontParams.flags = FONS_NVG_ZERO_TOPLEFT;
	fontParams.renderCreate = NULL;
	fontParams.renderUpdate = NULL;
	fontParams.renderDraw = NULL;
	fontParams.renderDelete = NULL;
	fontParams.userPtr = NULL;
	ctx->fs = fonsNVGCreateInternal(&fontParams);
...
void nvgStrokePaint(NVGcontext* ctx, NVGpaint paint)
{
	...
	ctx->xformMultFn(state->stroke.xform);
}
...
void nvgFillPaint(NVGcontext* ctx, NVGpaint paint)
{
	NVGstate* state = nvg__getState(ctx);
	state->fill = paint;
	ctx->xformMultFn(state->fill.xform);
...
static void nvg__appendCommands(NVGcontext* ctx, float* vals, int nvals)
{
	...
	while (i < nvals) {
		...
		switch (cmd) {
		case NVG_MOVETO:
			ctx->xformFn(&vals[i+1],&vals[i+2]/*, state->xform*/, vals[i+1],vals[i+2]);
			i += 3;
			break;
		case NVG_LINETO:
			ctx->xformFn(&vals[i+1],&vals[i+2]/*, state->xform*/, vals[i+1],vals[i+2]);
		...
		case NVG_BEZIERTO:
			ctx->xformFn(&vals[i+1],&vals[i+2]/*, state->xform*/, vals[i+1],vals[i+2]);
			ctx->xformFn(&vals[i+3],&vals[i+4]/*, state->xform*/, vals[i+3],vals[i+4]);
			ctx->xformFn(&vals[i+5],&vals[i+6]/*, state->xform*/, vals[i+5],vals[i+6]);
			i += 7;
			break;
		case NVG_CLOSE:
			i++;
...
{
	return fonsNVGAddFont(ctx->fs, name, filename, 0);
}

int nvgCreateFontAtIndex(NVGcontext* ctx, const char* name, const char* filename, const int fontIndex)
{
	return fonsNVGAddFont(ctx->fs, name, filename, fontIndex);
}

int nvgCreateFontMem(NVGcontext* ctx, const char* name, unsigned char* data, int ndata, int freeData)
{
	return fonsNVGAddFontMem(ctx->fs, name, data, ndata, freeData, 0);
}

int nvgCreateFontMemAtIndex(NVGcontext* ctx, const char* name, unsigned char* data, int ndata, int freeData, const int fontIndex)
{
	return fonsNVGAddFontMem(ctx->fs, name, data, ndata, freeData, fontIndex);
}

int nvgFindFont(NVGcontext* ctx, const char* name)
{
	if (name == NULL) return -1;
	return fonsNVGGetFontByName(ctx->fs, name);
}
...
int nvgAddFallbackFontId(NVGcontext* ctx, int baseFont, int fallbackFont)
{
	...
	return fonsNVGAddFallbackFont(ctx->fs, baseFont, fallbackFont);
}
...
{
	fonsNVGResetFallbackFont(ctx->fs, baseFont);
}
...
{
	NVGstate* state = nvg__getState(ctx);
	state->fontId = fonsNVGGetFontByName(ctx->fs, font);
}
...
static void nvg__flushTextTexture(NVGcontext* ctx)
{
	...
	if (fonsNVGValidateTexture(ctx->fs, dirty)) {
		int fontImage = ctx->fontImages[ctx->fontImageIdx];
		// Update texture
		if (fontImage != 0) {
			int iw, ih;
			const unsigned char* data = fonsNVGGetTextureData(ctx->fs, &iw, &ih);
			int x = dirty[0];
			int y = dirty[1];
...
static int nvg__allocTextAtlas(NVGcontext* ctx)
{
	...
	++ctx->fontImageIdx;
	fonsNVGResetAtlas(ctx->fs, iw, ih);
	return 1;
}
...
{
	NVGstate* state = nvg__getState(ctx);
	FONS_NVGtextIter iter, prevIter;
	FONS_NVGquad q;
	NVGvertex* verts;
	...
	if (state->fontId == FONS_NVG_INVALID) return x;

	fonsNVGSetSize(ctx->fs, state->fontSize*scale);
	fonsNVGSetSpacing(ctx->fs, state->letterSpacing*scale);
	fonsNVGSetBlur(ctx->fs, state->fontBlur*scale);
	fonsNVGSetAlign(ctx->fs, state->textAlign);
	fonsNVGSetFont(ctx->fs, state->fontId);

	cverts = nvg__maxi(2, (int)(end - string)) * 6; // conservative estimate.
	verts = nvg__allocTempVerts(ctx, cverts);
	if (verts == NULL) return x;

	fonsNVGTextIterInit(ctx->fs, &iter, x*scale, y*scale, string, end, FONS_NVG_GLYPH_BITMAP_REQUIRED);
	
	prevIter = iter;
	while (fonsNVGTextIterNext(ctx->fs, &iter, &q)) {
		float c[4*2];
		if (iter.prevGlyphIndex == -1) { // can not retrieve glyph?
			...
			if (!nvg__allocTextAtlas(ctx))
				break; // no memory :(
			iter = prevIter;
			fonsNVGTextIterNext(ctx->fs, &iter, &q); // try again
			if (iter.prevGlyphIndex == -1) // still can not find glyph?
				break;
		}
		prevIter = iter;
		// Transform corners.
		ctx->xformFn(&c[0],&c[1], q.x0*invscale, q.y0*invscale);
		ctx->xformFn(&c[2],&c[3], q.x1*invscale, q.y0*invscale);
		ctx->xformFn(&c[4],&c[5], q.x1*invscale, q.y1*invscale);
		ctx->xformFn(&c[6],&c[7], q.x0*invscale, q.y1*invscale);
		// Create triangles
		if (nverts+6 <= cverts) {
...
void nvgTextBox(NVGcontext* ctx, float x, float y, float breakRowWidth, const char* string, const char* end)
{
	...
	if (state->fontId == FONS_NVG_INVALID) return;
...
int nvgTextGlyphPositions(NVGcontext* ctx, float x, float y, const char* string, const char* end, NVGglyphPosition* positions, int maxPositions)
{
	...
	if (state->fontId == FONS_NVG_INVALID) return 0;
	...
	fonsNVGTextIterInit(ctx->fs, &iter, x*scale, y*scale, string, end, FONS_NVG_GLYPH_BITMAP_OPTIONAL);
	prevIter = iter;
	while (fonsNVGTextIterNext(ctx->fs, &iter, &q)) {
...
int nvgTextBreakLines(NVGcontext* ctx, const char* string, const char* end, float breakRowWidth, NVGtextRow* rows, int maxRows)
{
	...
	fonsNVGTextIterInit(ctx->fs, &iter, 0, 0, string, end, FONS_NVG_GLYPH_BITMAP_OPTIONAL);
	prevIter = iter;
	while (fonsNVGTextIterNext(ctx->fs, &iter, &q)) {
		if (iter.prevGlyphIndex < 0 && nvg__allocTextAtlas(ctx)) { // can not retrieve glyph?
			iter = prevIter;
...
float nvgTextBounds(NVGcontext* ctx, float x, float y, const char* string, const char* end, float* bounds)
{
	...
	width = fonsNVGTextBounds(ctx->fs, x*scale, y*scale, string, end, bounds);
	if (bounds != NULL) {
		// Use line bounds for height.
		fonsNVGLineBounds(ctx->fs, y*scale, &bounds[1], &bounds[3]);
		bounds[0] *= invscale;
		bounds[1] *= invscale;
		bounds[2] *= invscale;
		bounds[3] *= invscale;
...
void nvgTextBoxBounds(NVGcontext* ctx, float x, float y, float breakRowWidth, const char* string, const char* end, float* bounds)
{
	...
	if (state->fontId == FONS_NVG_INVALID) {
		if (bounds != NULL)
	...
	fonsNVGSetAlign(ctx->fs, state->textAlign);
	fonsNVGSetFont(ctx->fs, state->fontId);
	fonsNVGLineBounds(ctx->fs, 0, &rminy, &rmaxy);
	rminy *= invscale;
	rmaxy *= invscale;
...
void nvgTextMetrics(NVGcontext* ctx, float* ascender, float* descender, float* lineh)
{
	...
	fonsNVGVertMetrics(ctx->fs, ascender, descender, lineh);
	if (ascender != NULL)
		*ascender *= invscale;
	if (descender != NULL)
		*descender *= invscale;
