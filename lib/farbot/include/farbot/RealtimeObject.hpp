...
public:
	...
	std::enable_if_t<O == RealtimeObjectOptions::realtimeMutatable, std::void_t<Args...>> realtimeReplace(Args &&...args) noexcept {
     ...
     */
	template <RealtimeObjectOptions O = Options, typename... Args>
	std::enable_if_t<O == RealtimeObjectOptions::nonRealtimeMutatable, std::void_t<Args...>> nonRealtimeReplace(Args &&...args) {
	...
	// marek added these
	[[nodiscard]] auto scopedRead() noexcept { return ScopedAccess<ThreadType::nonRealtime>(*this); }
	[[nodiscard]] auto scopedWrite() noexcept { return ScopedAccess<ThreadType::realtime>(*this); }

private:
	using Impl =
		std::conditional_t<Options == RealtimeObjectOptions::realtimeMutatable, detail::RealtimeMutatable<T>, detail::NonRealtimeMutatable<T>>;
	Impl mImpl;
};
