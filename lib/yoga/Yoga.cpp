...
static void YGNodePrintInternal(
  ...
  YLog::log(node, YGLogLevelDebug, nullptr, str.c_str());
}
  ...
  if (!needToVisitNode && cachedResults != nullptr) {
    ...
    if (cachedResults == nullptr) {
      ...
      if (layout->nextCachedMeasurementsIndex == YG_MAX_CACHED_RESULT_COUNT) {
        if (gPrintChanges) {
          YLog::log(node, YGLogLevelVerbose, nullptr, "Out of cache entries!\n");
        }
        layout->nextCachedMeasurementsIndex = 0;
      }
...
void YGAssert(const bool condition, const char* message) {
  if (!condition) {
    YLog::log(YGNodeRef{nullptr}, YGLogLevelFatal, nullptr, "%s\n", message);
...
void YGAssertWithNode(
    ...
    const char* message) {
  if (!condition) {
    YLog::log(node, YGLogLevelFatal, nullptr, "%s\n", message);
    throwLogicalErrorWithMessage(message);
  }
}
...
void YGAssertWithConfig(
    ...
    const char* message) {
  if (!condition) {
    YLog::log(config, YGLogLevelFatal, nullptr, "%s\n", message);
    throwLogicalErrorWithMessage(message);
  }
}
