...
public:
	vec4 color;
	PadButton(Graphics &g,
			  int pos,
			  glm::vec4 upColor	  = {0.5, 0.5, 0.5, 1.0},
			  glm::vec4 downColor = {0.8, 0.8, 0.8, 1.0})
		: Layer(g, "")
		, downColor(downColor) {
		interactive = true;
		down		= false;
...
void PadGrid::setNumPads(int cols, int rows) {
	...
	for (int y = 0; y < rows; y++) {
		for (int x = 0; x < cols; x++) {
			...
			addChild(b);
			b->buttonPressed = [i, this]() {
				if (padDown) padDown(i);
			};
			b->buttonReleased = [i, this]() {
				if (padUp) padUp(i);
			};
