...
public:
	...
	struct Settings {
		Settings(const Dialogs &dialogs) : dialogs(dialogs) {}
	...
	ScrollingListDeletableView(Graphics &g, Settings &settings, std::shared_ptr<ScrollingListItem> item) : ScrollingListItemView(g, item), settings(settings) {}
...
public:
	ScrollingListDeletableStringView(Graphics &g, Settings &settings, std::shared_ptr<ScrollingListItem> item) : ScrollingListDeletableView(g, settings, item) {}
	
	void draw() override {
		ScrollingListDeletableView::draw();
		if(collapsing) return;
		g.setColor(1);
		g.drawTextVerticallyCentred(item->name, glm::vec2(horizontalScroll + x+20 * g.pixelScale, y + height / 2));
	}
};
