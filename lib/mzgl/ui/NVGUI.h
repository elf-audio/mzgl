...
namespace NVGUI {
	...
	public:
		...
		NVG &nvg;
		NVGLayer(Graphics &g, NVG &nvg) : Layer(g), nvg(nvg) {}
	};
	...
	public:
		...
		Button(Graphics &g, NVG &nvg) : NVGLayer(g, nvg) {
			x = 100;
			y = 400;
			width = 150;
			height = 50;
	...
	//			nvg.fill({0.8, 0.8, 0.8, 1});
		...
		virtual void touchOver(float x, float y) override {
			over = inside(x, y);
		}
	...
	public:
		...
		: NVGLayer(g, nvg), min(min), max(max), value(value) {
	...
	//		// rail bg
			nvg.drawRoundedRect(Rectf(x + radius, y + radius - railThickness/2, width - radius * 2, railThickness), railThickness/2);
	//
	//		// highlight
			nvg.fill(hexColor(0x900000));
			nvg.drawRoundedRect(Rectf(x + radius, y + radius - railThickness/2,
							mapf(*value, min, max, 0, width - radius * 2), railThickness)
							, railThickness/2);
	...
	public:
		Toggle(Graphics &g, NVG &nvg) : NVGLayer(g, nvg) {
			interactive = true;
			width = 110;
			height = 60;
		}
	...
	public:
		Underlay(Graphics &g, NVG &nvg) : NVGLayer(g, nvg) {
			visible = false;
			amt = 0;
			interactive = false;
			shouldShow = false;
	...
	public:
		string name;
		Label(Graphics &g, NVG &nvg, string text) : NVGLayer(g, nvg), name(text) {}
		void draw() override {
	...
	public:
		...
		DropDown(Graphics &g, NVG &nvg, vector<string> options) : NVGLayer(g, nvg), options(options) {
			interactive = true;
			...
			if(out) {
				...
				for(int i = 0; i < options.size(); ++i) {
					...
					if(i==hoveredIndex || i==selectedIndex) {// highlight
						...
						if(i==0) {
							nvg.drawRoundedRect({x, y + originalHeight * i, width, originalHeight}, cornerRadius, cornerRadius, 0, 0);
						} else if(i==options.size()-1) {
							nvg.drawRoundedRect({x, y + originalHeight * i, width, originalHeight}, 0, 0, cornerRadius, cornerRadius);
						} else {
							nvg.drawRect({x, y + originalHeight * i + 1, width, originalHeight - 2});
						}
					}
					nvg.fill(textColor);
					nvg.drawText(options[i], {x + textIndent, y + (originalHeight * i*outAmt + originalHeight/2)}, -1, 0);
				}
			} else {
//				nvg.fill(bgColor);
//				nvg.drawRoundedRect({x, y, width, height}, originalHeight/4);
		...
		void touchOver(float x, float y) override {
			if(inside(x, y)) {
				hoveredIndex = clampf((y - this->y) / originalHeight, 0, options.size()-1);
			} else {
				hoveredIndex = -1;
			}
		}
		...
		bool touchDown(float x, float y, int id) override {
			if(!out) {
				...
				if(brokenOut) {
					...
					int newIndex = clampf((y - this->y) / originalHeight, 0, options.size()-1);
