...
public:
	...
	Layout(std::vector<Layout> childs) : children(childs) {}
	
	Layout(std::string dir, std::vector<Layout> childs) : direction(dir), children(childs) { assert(dir=="v" || dir=="h"); }
	
	Layout(std::string dir, float dim, std::vector<Layout> childs) :
		direction(dir),
		children(childs),
		dimension(dim),
		dimType(DimensionType::percent) { assert(dir=="v" || dir=="h"); }
	
	Layout(std::string dir, int dim, std::vector<Layout> childs) :
		direction(dir),
		children(childs),
		dimension(dim),
		dimType(DimensionType::pixels) { assert(dir=="v" || dir=="h"); }
	
	Layout() {}
	
	// if you set only one mode, it applies to both horizontal and vertial
	Layout(Layer *lay, Mode modeHorizontal = Mode::FIT_TO_PARENT,
		   Mode modeVertical = Mode::NONE
		   ) : layer(lay), modeHorizontal(modeHorizontal), modeVertical(modeVertical) {
	...
	Layout(float dim, Layer *lay) : layer(lay), dimension(dim), dimType(DimensionType::percent) {}
	
	Layout(int dim, Layer *lay) : layer(lay), dimension(dim), dimType(DimensionType::pixels) {}
