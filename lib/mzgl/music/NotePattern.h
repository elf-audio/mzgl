...
public:
	...
	string toString() {
		return statusString() + "     delay: " + to_string(delay) + "   pitch: " + to_string(pitch)
...
public:
	...
	bool operator!=(const NotePattern &other) const { return !((*this) == other); }
	...
	void lock() { patternMutex.lock(); }

	void unlock() { patternMutex.unlock(); }
	...
	void erase(const PatternNote &note) { notes.erase(note); }

	int getTimeOfBeatInSamples();

	int getPatternLengthInSamples() { return getTimeOfBeatInSamples() * beatsPerBar * numBars; }

	int getPatternLength() { return beatsPerBar * numBars * ppqn; }
