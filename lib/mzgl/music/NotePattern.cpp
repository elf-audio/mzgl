...
bool NotePattern::operator==(const NotePattern &other) const {
	...
	return containsAllTheSameNotes(other) && ((NotePattern &) other).containsAllTheSameNotes(*this);
}
		...
		if (n.pitch == pitch) {
			if (t >= start && t < start + n.length) { // normal notes
				return findIt;
					   ...
					   && (t >= start || t < start + n.length - patternLength
	...
	for (auto it = begin(); it != end(); it++) {
		...
		if (finish > start) {
			if (noteStart >= start && noteStart < finish) {
				noteCommands.push_back(
					NoteCommand(MIDI_NOTE_ON, noteStart - start, (*it).second.pitch, (*it).second.vel));
			}
			...
			if (noteStart >= start) {
				...
				noteCommands.push_back(NoteCommand(MIDI_NOTE_ON,
												   noteStart - (start - getPatternLengthInSamples()),
												   (*it).second.pitch,
												   (*it).second.vel));
			}
			...
			if (noteEnd >= start) {
				noteCommands.push_back(
					NoteCommand(MIDI_NOTE_OFF, noteEnd - start, (*it).second.pitch, (*it).second.vel));
			} else if (noteEnd < finish) {
				noteCommands.push_back(NoteCommand(MIDI_NOTE_OFF,
												   noteEnd - (start - getPatternLengthInSamples()),
												   (*it).second.pitch,
												   (*it).second.vel));
			}
		}
