...
bool copyDir(const fs::path &source, const fs::path &destination, std::string &errMsg) {
    try {
        ...
        if (!fs::exists(source) || !fs::is_directory(source)) {
            errMsg = "Source directory " + source.string() +
    ...
    if (fs::exists(p)) {
        ...
        for (int i = startingIndex; i < 10000; i++) {
            ...
            auto pp = p.parent_path() /
                      (filenameBase + "-" + std::to_string(i) + p.extension().string());
...
void updateDocumentsDirectory() {
    ...
    if (!fs::exists(documentsDir / "settings")) {
        Log::d() << "[TESTS]: Documents path, making settings:  "
                 << (documentsDir / "settings").string().c_str();
