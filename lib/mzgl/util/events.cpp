...
void addListener(int type, void *obj, function<void()> fn) {
	...
	switch (type) {
		case UPDATE: updateListeners.push_back(make_pair(obj, fn)); break;
		case DRAW: drawListeners.push_back(make_pair(obj, fn)); break;
	}
}
...
void removeListener(int type, void *obj) {
	switch (type) {
		case UPDATE: removeFromListeners(updateListeners, obj); break;
		case DRAW: removeFromListeners(drawListeners, obj); break;
	}
}
