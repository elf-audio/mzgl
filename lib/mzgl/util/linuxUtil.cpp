...
void linuxLoadFileDialog(string msg, const vector<string> &allowedExtensions,
...
// maybe we need this if the dialog doesn't go away?
    //gtk_widget_destroy (dialog);
}
...
void linuxTextboxDialog(std::string title, std::string msg, std::string text,
    ...
    // For some reason, for dialogs made with gtk_dialog_new_with_buttons
    // this is how to destroy it (rather than gtk_widget_destroy)
   ...
   // Log::d() << "Destroying dialog";

   // gtk_widget_destroy(GTK_WIDGET(dialog));
    //gtk_window_close(GTK_WINDOW(dialog));
   // Log::d() << "Destroyed dialog";
...
void linuxTwoOptionCancelDialog(std::string title, std::string msg,
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(title.c_str(),
                                                    nullptr,
                                                    (GtkDialogFlags) 0,
                                                    buttonOneText.c_str(), 1,
                                                    buttonTwoText.c_str(), 2,
                                                    "Cancel", 3,
