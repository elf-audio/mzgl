...
struct android_statics {
	...
	std::function<void()> exportComplete;
	std::function<void(float)> exportProgress;
	std::function<void(const std::string &)> exportFailed;
} android_statics;
...
void androidStartDataExport(const std::function<void()> &exportComplete,
							const std::function<void(float)> &exportProgress,
							const std::function<void(const std::string &)> &exportFailed) {
	android_statics.exportComplete = exportComplete;
	android_statics.exportProgress = exportProgress;
	android_statics.exportFailed   = exportFailed;
	callJNI("exportData");
}
...
std::string getAndroidExternalDataPath() {
	return callJNIForString("getDataDirectory");
}
...
JNIEXPORT void JNICALL Java_com_elf_MZGLActivity_exportProgressChanged(JNIEnv *env,
																	   jobject thiz,
																	   jfloat progress) {
	Log::d() << "Exporting progress " << progress;
	if (android_statics.exportProgress) {
		android_statics.exportProgress(progress);
	}
}

JNIEXPORT void JNICALL Java_com_elf_MZGLActivity_exportCompleted(JNIEnv *env, jobject thiz) {
	Log::d() << "Exporting Completed";
	if (android_statics.exportComplete) {
		android_statics.exportComplete();
	}

	android_statics.exportComplete = nullptr;
	android_statics.exportProgress = nullptr;
	android_statics.exportFailed   = nullptr;
}

JNIEXPORT void JNICALL Java_com_elf_MZGLActivity_exportFailed(JNIEnv *env, jobject thiz, jstring reason) {
	Log::d() << "Exporting failed" << jstringToString(env, reason);
	if (android_statics.exportFailed) {
		android_statics.exportFailed(jstringToString(env, reason));
	}
}
...
JNIEXPORT void JNICALL Java_com_elf_MZGLActivity_fileDownloadCancelled(JNIEnv *env, jobject thiz, jstring url) {
	androidGetApp()->main.runOnMainThread([download = jstringToString(env, url)]() {
...
JNIEXPORT void JNICALL Java_com_elf_MZGLActivity_fileDownloadProgress(JNIEnv *env,
																	  ...
																	  jfloat progress) {
	androidGetApp()->main.runOnMainThread(
		[download = jstringToString(env, url), progress]() { notifyFileDownloadProgress(download, progress); });
}
