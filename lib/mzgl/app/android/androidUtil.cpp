...
int getAudioInputCount(int32_t deviceId) {
	return callJNIForInt("getAudioInputChannelCount", deviceId);
}
	...
	if (auto impl = midiImpl.lock()) {
		if (isInput) {
			impl->inputDeviceConnected(
				std::make_shared<AndroidMidiDevice>(deviceName, deviceId, portId, MidiDevice::Direction::Input));
		} else {
			impl->outputDeviceConnected(
				std::make_shared<AndroidMidiDevice>(deviceName, deviceId, portId, MidiDevice::Direction::Output));
		}
	}
}
						...
						std::begin(devices), std::end(devices), [deviceId, portId](auto &&deviceToTest) {
							if (auto *androidInput = dynamic_cast<AndroidMidiDevice *>(deviceToTest.get())) {
								return androidInput->deviceIdentifier == deviceId
									   ...
									   && androidInput->direction == MidiDevice::Direction::Input;
							}
							return false;
						});
...
JNIEXPORT void JNICALL Java_com_elf_MZGLBleMidiManager_midiReceived(
	JNIEnv *env, jobject thiz, jbyteArray bytes, jlong timestamp, jint deviceId, jint portId) {
	androidParseMidiData(copyMidiBytes(env, bytes), timestamp, deviceId, portId);
}
