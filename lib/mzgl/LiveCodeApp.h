...
public:
	...
	void addFileToWatch(std::string path) {
		compiler.addFileToWatch(path);
	}
	
	LiveCodeApp(Graphics &g, std::string headerFile, std::shared_ptr<_AudioSystem> audioSystem = nullptr) : App(g), compiler(g) {
...
//			}
	...
	// KEYS
	void keyDown(int key) override { if(d) d->keyDown(key); }
	void keyUp	(int key) override { if(d) d->keyUp(key); }
	
	
	// TOUCHES
	void touchOver	(float x, float y) override { if(d) d->touchOver(x, y); }
	void touchDown	(float x, float y, int id) override { if(d) d->touchDown(x, y, id); }
	void touchMoved	(float x, float y, int id) override { if(d) d->touchMoved(x, y, id); }
	void touchUp	(float x, float y, int id) override { if(d) d->touchUp(x, y, id); }
	...
	// EVENTS
	void resized() override {if(d) d->resized();}
	
	// LIFE-CYCLE
	void setup() override {if(d) d->setup();}
	void draw() override { if(d) d->draw(); }
	void drawAfterUI() override { if(d) d->drawAfterUI(); }
