...
- (std::vector<std::string>)listDevices {
	...
	for (AVCaptureDevice *captureDevice in devices) {
		...
		Log::e() << "Device: " << i << ": " << deviceNames.back();
		i++;
	}
	return deviceNames;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput
	...
	if (grabberPtr != NULL) {
		@autoreleasepool {
			...
			if (grabberPtr != NULL
				&& grabberPtr->internalGlDataType
					   == GL_RGBA) { //GL_BGRA) { //<<-- marek changed this because couldn't find the declaration
...
AVFoundationVideoGrabber::~AVFoundationVideoGrabber() {
	Log::e() << "AVFoundationVideoGrabber dtor";
	bLock = true;
	if (grabber) {
	...
	if ([grabber initCapture:fps capWidth:w capHeight:h]) {
		...
		if (internalGlDataType == GL_RGB) {
			pixels = (GLubyte *) malloc(
				width * height * 3); //new unsigned char[width * width * 3];//memset(pixels, 0, width*height*3);
...
std::vector<std::string> AVFoundationVideoGrabber::listDevices() {
