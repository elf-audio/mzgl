...
void checkForDevice(std::optional<AudioPort> &port, const std::vector<AudioPort> &ports, const std::string &type) {
	if (!port.has_value()) {
		return;
	}

	if (std::find_if(
			ports.begin(), ports.end(), [&](auto &&possiblePort) { return possiblePort.portId == port->portId; })
		== ports.end()) {
		Log::e() << "CoreAudio: " << type << " Port " << port->name << " is no longer available";
		port.reset();
	}
}

void CoreAudioSystem::checkDeviceAvailability() {
	checkForDevice(inputPort, inputPorts, "Input");
	checkForDevice(outputPort, outputPorts, "Output");

	if (!inputPort.has_value()) {
		for (auto &port: inputPorts) {
			if (port.isDefaultInput) {
				inputPort = port;
				Log::d() << "CoreAudio: Using default input port: " << inputPort->name;
				break;
			}
		}
	}
	...
	if (!outputPort.has_value()) {
		for (auto &port: outputPorts) {
			if (port.isDefaultOutput) {
				outputPort = port;
				Log::d() << "CoreAudio: Using default output port: " << outputPort->name;
				break;
			}
		}
	}
