...
int AudioSystemRtAudio_callback( void *outputBuffer, void *inputBuffer, unsigned int nBufferFrames,
...
//	printDevices();
	...
	if(this->numInChannels > 0) {
        ...
        if(iParams->nChannels!=dev.inputChannels) {
            Log::e() << "Default input device '" << dev.name
            << "' doesn't have " << iParams->nChannels
            <<
...
//		}
		...
		audio->openStream( oParams, iParams, RTAUDIO_FLOAT32, sampleRate, &bufferSize, &AudioSystemRtAudio_callback, (void *)this, &opts );
