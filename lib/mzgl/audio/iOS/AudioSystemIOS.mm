...
static OSStatus audioSystemInRenderCallback(void *inRefCon,
	...
	if(context->stream->inputCallback) {
		...
		context->stream->inputCallback((float *)bufferList->mBuffers[0].mData, bufferList->mBuffers[0].mDataByteSize / sizeof(Float32),
...
void AudioSystemIOS::configureAudioSession() {
	...
	if([audioSession respondsToSelector:@selector(setCategory:withOptions:error:)]) {
		...
		if(![audioSession setCategory: category
						  withOptions: opts
								error:&err]) {
...
static void routeChangeNotificationHandler(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
...
AudioSystemIOS::AudioSystemIOS() {
	CFNotificationCenterAddObserver(CFNotificationCenterGetLocalCenter()
									...
									, this, routeChangeNotificationHandler,
									(__bridge CFStringRef) AVAudioSessionRouteChangeNotification, NULL, CFNotificationSuspensionBehaviorDeliverImmediately);
...
vector<AudioPort> AudioSystemIOS::getOutputs() {
	...
	for(AVAudioSessionPortDescription *desc in descs) {
		...
		for(AVAudioSessionChannelDescription *chan in desc.channels) {
            NSLog(@"CHANNEL %@ , num %lu label: %u", chan.channelName, (unsigned long)chan.channelNumber, (unsigned int)chan.channelLabel);
		}
		ports.push_back(p);
	}
