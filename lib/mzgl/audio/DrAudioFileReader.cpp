...
public:
	...
	virtual uint32_t getNumChannels() override { return numChannels; }

	virtual uint32_t getSampleRate() override { return infile.fmt.sampleRate; }

	virtual uint32_t read(vector<float> &outSamples) override {
		uint32_t framesRead =
			drwav_read_pcm_frames_f32(&infile, outSamples.size() / numChannels, outSamples.data());
		if (framesRead != outSamples.size() / numChannels) outSamples.resize(framesRead * numChannels);
		return framesRead;
	}

	virtual void close() override { drwav_uninit(&infile); }
...
public:
	...
	virtual uint32_t getNumChannels() override { return infile->channels; }

	virtual uint32_t getSampleRate() override { return infile->sampleRate; }

	virtual uint32_t read(vector<float> &outSamples) override {
		uint32_t framesRead =
			drflac_read_pcm_frames_f32(infile, outSamples.size() / getNumChannels(), outSamples.data());
...
public:
	...
	virtual uint32_t read(vector<float> &outSamples) override {
		uint32_t framesRead =
			drmp3_read_pcm_frames_f32(&infile, outSamples.size() / getNumChannels(), outSamples.data());
		if (framesRead != outSamples.size() / getNumChannels()) outSamples.resize(framesRead * getNumChannels());
		return framesRead;
	}

	virtual void close() override { drmp3_uninit(&infile); }
