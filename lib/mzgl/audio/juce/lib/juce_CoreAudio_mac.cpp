...
#	define JUCE_COREAUDIOLOG(a)
	...
	static auto getDataPtrAndSize(ManagedAudioBufferList::Ref &t) {
		const auto size =
			t.get() != nullptr ? ManagedAudioBufferList::storageSizeForNumBuffers(t->mNumberBuffers) : 0;
	...
	[[nodiscard]] static auto audioObjectGetProperty(AudioObjectID objectID,
		...
		if (auto status = AudioObjectGetPropertyData(objectID, &address, 0, nullptr, &size, ptr);
			status != noErr) {
			onError(status);
			return Result {};
		}
	...
	struct CoreAudioClasses {
		...
		public:
			...
			bool updateDetailsFromDevice(const BigInteger &activeIns, const BigInteger &activeOuts) {
				...
				auto newOutput =
					rawToUniquePtr(outStream != nullptr ? new Stream(false, *this, activeOuts) : nullptr);
			...
			//==============================================================================
			String reopen(const BigInteger &ins,
			...
			void audioCallback(const AudioTimeStamp *inputTimestamp,
				...
				if (callback != nullptr) {
					...
					for (int i = numOutputChans; --i >= 0;) {
						...
						auto dest =
							((float *) outOutputData->mBuffers[info.streamNum].mData) + info.dataOffsetSamples;
						auto stride = info.dataStrideSamples;
			...
			void deviceDetailsChanged() {
				if (callbacksAllowed.get() == 1) startTimer(100);
			}
			...
			struct Stream {
						...
						for (int i = 0; i < numStreams; ++i) {
							...
							for (unsigned int j = 0; j < b.mNumberChannels; ++j) {
								...
								if (auto opt = visitor(Args {
										i, static_cast<int>(j), chanNum++, static_cast<int>(b.mNumberChannels)}))
				...
				static StringArray getChannelNames(bool isInput, CoreAudioInternal &parent) {
					auto names = visitChannels(isInput, parent, [&](const auto &args) -> std::optional<String> {
						...
						if (name.isEmpty()) name << (isInput ? "Input " : "Output ") << (args.chanNum + 1);
				...
				static int getLatencyFromDevice(bool isInput, CoreAudioInternal &parent) {
					...
					const auto deviceLatency =
						audioObjectGetProperty<UInt32>(
							parent.deviceID,
							{kAudioDevicePropertyLatency, scope, juceAudioObjectPropertyElementMain})
							.value_or(0);
					...
					const auto framesInBuffer =
						audioObjectGetProperty<UInt32>(parent.deviceID,
													   {kAudioDevicePropertyBufferFrameSize,
														kAudioObjectPropertyScopeWildcard,
														juceAudioObjectPropertyElementMain})
							.value_or(0);
					...
					if (auto streams = audioObjectGetProperties<AudioStreamID>(
							parent.deviceID,
							{kAudioDevicePropertyStreams, scope, juceAudioObjectPropertyElementMain});
						!streams.empty())
						streamLatency =
							audioObjectGetProperty<UInt32>(
								streams.front(),
				...
				bool checkTimestampsForDiscontinuity(const AudioTimeStamp *timestamp) noexcept {
					if (channels > 0) {
						...
						if (timestamp != nullptr
							&& std::fabs(previousSampleTime - timestamp->mSampleTime) >= 1.0) {
			...
			static int getLatency(const std::unique_ptr<Stream> &ptr) {
				return getWithDefault(ptr, &Stream::latency);
			}
		...
		private:
			...
			public:
				...
				ScopedAudioDeviceIOProcID(CoreAudioInternal &coreAudio,
										  AudioDeviceID d,
										  AudioDeviceIOProc audioIOProc)
					: deviceID(d) {
		...
		public:
			CoreAudioIODevice(CoreAudioIODeviceType *dt,
				...
				internal = [this, &inputDeviceId, &outputDeviceId] {
					if (outputDeviceId == 0 || outputDeviceId == inputDeviceId) {
						...
						return std::make_unique<CoreAudioInternal>(
							*this, inputDeviceId, true, outputDeviceId != 0);
					}
				...
				AudioObjectAddPropertyListener(
					kAudioObjectSystemObject, &pa, hardwareListenerProc, internal.get());
			}
			...
			int getIndexOfDevice(bool asInput) const {
				return deviceType->getDeviceNames(asInput).indexOf(getName());
			}
		...
		private:
			...
			void inputAudioCallback(const float *const *channels,
...

				{
					...
					if (callback != nullptr) {
						const auto numActiveOutputChannels =
							outputWrapper.getActiveChannels().countNumberOfSetBits();
			...
			void handleAudioDeviceAboutToStart(AudioIODevice *device) {
				...
				if (anySampleRateChanges && owner != nullptr) owner->audioDeviceListChanged();
			...
			struct DeviceWrapper final : public AudioIODeviceCallback {
				...
				void start(AudioIODeviceCallback *callbackToNotify) const {
					return device->start(callbackToNotify);
				}
				...
				void updateSampleTimeFromContext(const AudioIODeviceCallbackContext &context) noexcept {
					auto callbackSampleTime =
						context.hostTimeNs != nullptr ? nsToSampleTime(*context.hostTimeNs) : 0;
    ...
    */
		...
		private:
			...
			static int getNumChannels(AudioDeviceID deviceID, bool input) {
				...
				if (auto bufList =
						audioObjectGetProperty<AudioBufferList>(deviceID,
																{kAudioDevicePropertyStreamConfiguration,
