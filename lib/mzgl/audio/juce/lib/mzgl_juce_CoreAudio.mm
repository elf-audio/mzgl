...
public:
	struct Deleter {
		void operator()(ManagedAudioBufferList *p) const {
			if (p != nullptr) p->~ManagedAudioBufferList();
...
private:
	...
	explicit ManagedAudioBufferList(std::size_t numBuffers) { mNumberBuffers = static_cast<UInt32>(numBuffers); }

	static constexpr auto audioBufferListHeaderSize = sizeof(AudioBufferList) - sizeof(::AudioBuffer);

	// Delete the copy constructor
	ManagedAudioBufferList(const ManagedAudioBufferList &) = delete;

	// Delete the copy assignment operator
	ManagedAudioBufferList &operator=(const ManagedAudioBufferList &) = delete;
...
static auto getDataPtrAndSize(ManagedAudioBufferList::Ref &t) {
	const auto size = t.get() != nullptr ? ManagedAudioBufferList::storageSizeForNumBuffers(t->mNumberBuffers) : 0;
	return std::make_tuple(t.get(), (UInt32) size);
}

//==============================================================================
[[nodiscard]] static bool audioObjectHasProperty(AudioObjectID objectID,
...
[[nodiscard]] static auto audioObjectGetProperty(AudioObjectID objectID,
												 ...
												 OnError &&onError = {}) {
	using Result =
		std::conditional_t<std::is_same_v<T, AudioBufferList>, ManagedAudioBufferList::Ref, std::optional<T>>;

	if (!audioObjectHasProperty(objectID, address)) return Result {};

	auto result = [&] {
		if constexpr (std::is_same_v<T, AudioBufferList>) {
			UInt32 size {};

			if (auto status = AudioObjectGetPropertyDataSize(objectID, &address, 0, nullptr, &size);
				status != noErr) {
				onError(status);
				return Result {};
			}
...
static bool audioObjectSetProperty(AudioObjectID objectID,
	...
	if (auto status =
			AudioObjectSetPropertyData(objectID, &address, 0, nullptr, static_cast<UInt32>(sizeof(T)), &value);
		status != noErr) {
		onError(status);
		return false;
	}
...
[[nodiscard]] static std::vector<T> audioObjectGetProperties(AudioObjectID objectID,
															 const AudioObjectPropertyAddress address,
															 OnError &&onError = {}) {
	if (!audioObjectHasProperty(objectID, address)) return {};
	...
	if (auto status = AudioObjectGetPropertyData(objectID, &address, 0, nullptr, &size, result.data());
		status != noErr) {
		onError(status);
		return {};
	}
...
struct SystemVol {
	explicit SystemVol(AudioObjectPropertySelector selector) noexcept
		: outputDeviceID(audioObjectGetProperty<AudioDeviceID>(kAudioObjectSystemObject,
	...
	float getGain() const noexcept { return audioObjectGetProperty<Float32>(outputDeviceID, addr).value_or(0.0f); }

	bool setGain(float gain) const noexcept {
		return audioObjectSetProperty(outputDeviceID, addr, static_cast<Float32>(gain));
	}

	bool isMuted() const noexcept { return audioObjectGetProperty<UInt32>(outputDeviceID, addr).value_or(0) != 0; }
...
bool JUCE_CALLTYPE SystemAudioVolume::setGain(float gain) {
	return SystemVol(juceAudioHardwareServiceDeviceProperty_VirtualMainVolume).setGain(gain);
}
...
struct CoreAudioClasses {
	...
	private:
		...
		auto err2log() const {
			return [this](OSStatus err) { OK(err); };
		}
	...
	public:
		...
		void allocateTempBuffers() {
			...
			for (auto *stream: streams)
				channels +=
					stream != nullptr ? stream->allocateTempBuffers(tempBufSize, channels, audioBuffer) : 0;
		}
		...
		Array<double> getSampleRatesFromDevice() const {
			...
			if (auto ranges =
					audioObjectGetProperties<AudioValueRange>(deviceID,
			...
			if (newSampleRates.isEmpty() && sampleRate > 0) newSampleRates.add(sampleRate);
		...
		Array<int> getBufferSizesFromDevice() const {
			...
			if (auto ranges = audioObjectGetProperties<AudioValueRange>(deviceID,
				...
				if (bufferSize > 0) newBufferSizes.addIfNotAlreadyThere(bufferSize);
			}

			if (newBufferSizes.isEmpty() && bufferSize > 0) newBufferSizes.add(bufferSize);

			return newBufferSizes;
		}

		int getFrameSizeFromDevice() const {
			return static_cast<int>(audioObjectGetProperty<UInt32>(deviceID,
		...
		bool isDeviceAlive() const {
			return deviceID != 0
				   && audioObjectGetProperty<UInt32>(deviceID,
		...
		bool updateDetailsFromDevice(const BigInteger &activeIns, const BigInteger &activeOuts) {
			...
			auto newInput =
				std::unique_ptr<Stream>(inStream != nullptr ? new Stream(true, *this, activeIns) : nullptr);
			auto newOutput =
				std::unique_ptr<Stream>(outStream != nullptr ? new Stream(false, *this, activeOuts) : nullptr);
		...
		StringArray getDeviceDetails() {
			...
			for (auto &s: sampleRates)
				availableSampleRates += " " + std::to_string(s);
			...
			for (auto &b: bufferSizes)
				availableBufferSizes += " " + std::to_string(b); //<< " " << b;
		...
		StringArray getSources(bool input) {
			StringArray s;
			auto types = audioObjectGetProperties<OSType>(deviceID,
			...
			for (auto type: types) {
				...
				if (OK(AudioObjectGetPropertyData(deviceID, &pa, 0, nullptr, &transSize, &avt))) s.add(buffer);
			}
		...
		int getCurrentSourceIndex(bool input) const {
			if (deviceID != 0) {
				if (auto currentSourceID = audioObjectGetProperty<OSType>(
						deviceID,
						{kAudioDevicePropertyDataSource, getScope(input), juceAudioObjectPropertyElementMain},
						err2log())) {
					auto types = audioObjectGetProperties<OSType>(deviceID,
																  {kAudioDevicePropertyDataSources,
																   kAudioObjectPropertyScopeWildcard,
		...
		double getNominalSampleRate() const {
			return static_cast<double>(audioObjectGetProperty<Float64>(deviceID,
		...
		bool setNominalSampleRate(double newSampleRate) const {
			if (std::abs(getNominalSampleRate() - newSampleRate) < 1.0) return true;

			return audioObjectSetProperty(deviceID,
										  {kAudioDevicePropertyNominalSampleRate,
										   kAudioObjectPropertyScopeGlobal,
										   juceAudioObjectPropertyElementMain},
										  static_cast<Float64>(newSampleRate),
										  err2log());
		}
		...
		String reopen(const BigInteger &ins, const BigInteger &outs, double newSampleRate, int bufferSizeSamples) {
			...
			if (!audioObjectSetProperty(deviceID,
										{kAudioDevicePropertyBufferFrameSize,
										 kAudioObjectPropertyScopeGlobal,
										 juceAudioObjectPropertyElementMain},
										static_cast<UInt32>(bufferSizeSamples),
										err2log())) {
				updateDetailsFromDevice(ins, outs);
			...
			if (sampleRates.size() == 0) return "Device has no available sample-rates";

			if (bufferSizes.size() == 0) return "Device has no available buffer-sizes";
		...
		bool start(AudioIODeviceCallback *callbackToNotify) {
			...
			for (auto *stream: getStreams())
				if (stream != nullptr) stream->previousSampleTime = invalidSampleTime;
			...
			if (scopedProcID.get() == nullptr && deviceID != 0) {
				scopedProcID = [&self	   = *this,
					...
					if (auto *procID = nextProcID.get()) {
						...
						if (self.OK(AudioDeviceStart(dID, procID))) return std::move(nextProcID);
					}
		...
		AudioIODeviceCallback *stop(bool leaveInterruptRunning) {
			...
			if (scopedProcID.get() != nullptr && (deviceID != 0) && !leaveInterruptRunning) {
				...
				for (int i = 40; --i >= 0;) {
					if (audioDeviceStopPending == false) break;
		...
		void audioCallback(const AudioTimeStamp *inputTimestamp,
			...
			if (audioDeviceStopPending) {
				if (OK(AudioDeviceStop(deviceID, scopedProcID.get()))) audioDeviceStopPending = false;
			...
			if (callback != nullptr) {
				for (int i = numInputChans; --i >= 0;) {
					...
					auto src =
						((const float *) inInputData->mBuffers[info.streamNum].mData) + info.dataOffsetSamples;
				...
				for (auto *stream: getStreams())
					if (stream != nullptr)
						owner.hadDiscontinuity |= stream->checkTimestampsForDiscontinuity(
							stream == inStream.get() ? inputTimestamp : outputTimestamp);

				const auto *timeStamp = numOutputChans > 0 ? outputTimestamp : inputTimestamp;
				const auto nanos =
					timeStamp != nullptr ? timeConversions.hostTimeToNanos(timeStamp->mHostTime) : 0;
			...
			for (auto *stream: getStreams())
				if (stream != nullptr) stream->previousSampleTime += static_cast<Float64>(bufferSize);
		}

		// called by callbacks (possibly off the main thread)
		void deviceDetailsChanged() {
			if (callbacksAllowed == 1) startTimer(100);
		}
		...
		struct Stream {
			...
			int allocateTempBuffers(int tempBufSize, int channelCount, HeapBlock<float> &buffer) {
				...
				for (int i = 0; i < channels; ++i)
					tempBuffers[i] = buffer.get() + channelCount++ * tempBufSize;
			...
			static auto visitChannels(bool isInput, CoreAudioInternal &parent, Visitor &&visitor) {
				...
				if (auto bufList =
						audioObjectGetProperty<AudioBufferList>(parent.deviceID,
					...
					for (int i = 0; i < numStreams; ++i) {
						...
						for (unsigned int j = 0; j < b.mNumberChannels; ++j) {
							...
							if (auto opt = visitor(
									Args {i, static_cast<int>(j), chanNum++, static_cast<int>(b.mNumberChannels)}))
								result.emplace_back(std::move(*opt));
						}
					}
				}
			...
			static Array<CallbackDetailsForChannel>
				getChannelInfos(bool isInput, CoreAudioInternal &parent, const BigInteger &active) {
				return visitChannels(
					isInput, parent, [&](const auto &args) -> std::optional<CallbackDetailsForChannel> {
						if (!active[args.chanNum]) return {};
			...
			static StringArray getChannelNames(bool isInput, CoreAudioInternal &parent) {
				auto names = visitChannels(isInput, parent, [&](const auto &args) -> std::optional<String> {
					...
					if (auto nameNSString =
							audioObjectGetProperty<NSString *>(
								parent.deviceID, {kAudioObjectPropertyElementName, getScope(isInput), element})
								.value_or(nullptr)) {
					...
					if (name.isEmpty()) {
						name += (isInput ? "Input " : "Output ") + std::to_string(args.chanNum + 1);
					}
			...
			static int getBitDepthFromDevice(bool isInput, CoreAudioInternal &parent) {
				return static_cast<int>(
					audioObjectGetProperty<AudioStreamBasicDescription>(parent.deviceID,
			...
			static int getLatencyFromDevice(bool isInput, CoreAudioInternal &parent) {
				...
				const auto deviceLatency =
					audioObjectGetProperty<UInt32>(
						parent.deviceID, {kAudioDevicePropertyLatency, scope, juceAudioObjectPropertyElementMain})
						.value_or(0);

				const auto safetyOffset =
					audioObjectGetProperty<UInt32>(
						parent.deviceID,
						{kAudioDevicePropertySafetyOffset, scope, juceAudioObjectPropertyElementMain})
						.value_or(0);

				const auto framesInBuffer = audioObjectGetProperty<UInt32>(parent.deviceID,
																		   {kAudioDevicePropertyBufferFrameSize,
																			kAudioObjectPropertyScopeWildcard,
																			juceAudioObjectPropertyElementMain})
												.value_or(0);
				...
				if (auto streams = audioObjectGetProperties<AudioStreamID>(
						parent.deviceID, {kAudioDevicePropertyStreams, scope, juceAudioObjectPropertyElementMain});
					!streams.empty())
					streamLatency = audioObjectGetProperty<UInt32>(
										streams.front(),
										{kAudioStreamPropertyLatency, scope, juceAudioObjectPropertyElementMain})
										.value_or(0);
		...
		static int getBitDepth(const std::unique_ptr<Stream> &ptr) {
			return getWithDefault(ptr, &Stream::bitDepth);
		}
		static int getChannels(const std::unique_ptr<Stream> &ptr) {
			return getWithDefault(ptr, &Stream::channels);
		}
		static int getNumChannelNames(const std::unique_ptr<Stream> &ptr) {
			return getWithDefault(ptr, &Stream::chanNames).size();
		}
		static String getChannelNames(const std::unique_ptr<Stream> &ptr) {
			return getWithDefault(ptr, &Stream::chanNames).joinIntoString(" ");
		}
		static BigInteger getActiveChannels(const std::unique_ptr<Stream> &ptr) {
			return getWithDefault(ptr, &Stream::activeChans);
		}
		static float **getTempBuffers(const std::unique_ptr<Stream> &ptr) {
			return getWithDefault(ptr, [](auto &s) { return s.tempBuffers.get(); });
		}
	...
	private:
		...
		public:
			...
			ScopedAudioDeviceIOProcID(CoreAudioInternal &coreAudio, AudioDeviceID d, AudioDeviceIOProc audioIOProc)
				: deviceID(d) {
				if (!coreAudio.OK(AudioDeviceCreateIOProcID(deviceID, audioIOProc, &coreAudio, &proc))) proc = {};
			}

			~ScopedAudioDeviceIOProcID() noexcept {
				if (proc != AudioDeviceIOProcID {}) AudioDeviceDestroyIOProcID(deviceID, proc);
			}

			ScopedAudioDeviceIOProcID(ScopedAudioDeviceIOProcID &&other) noexcept { swap(other); }
		...
		void timerCallback() override {
			...
			if (!updateDetailsFromDevice()) owner.stopInternal();
			else if ((oldBufferSize != bufferSize || !approximatelyEqual(oldSampleRate, sampleRate))
					 && owner.shouldRestartDevice())
				owner.restart();
		}

		void handleAsyncUpdate() override {
			if (auto devType = owner.deviceType.lock()) {
				devType->audioDeviceListChanged();
			}
			//			if (owner.deviceType != nullptr) owner.deviceType->audioDeviceListChanged();
		}
		...
		static OSStatus audioIOProc(AudioDeviceID /*inDevice*/,
									...
									void *device) {
			static_cast<CoreAudioInternal *>(device)->audioCallback(
				inInputTime, inOutputTime, inInputData, outOutputData);
			return noErr;
		}
		...
		static OSStatus deviceListenerProc(AudioDeviceID /*inDevice*/,
			...
			const auto detailsChanged =
				std::any_of(pa, pa + numAddresses, [](const AudioObjectPropertyAddress &x) {
					...
					return std::find(std::begin(selectors), std::end(selectors), x.mSelector)
						   != std::end(selectors);
				});

			const auto requestedRestart =
				std::any_of(pa, pa + numAddresses, [](const AudioObjectPropertyAddress &x) {
					constexpr UInt32 selectors[] {
			...
			if (detailsChanged) intern.deviceDetailsChanged();

			if (requestedRestart) intern.deviceRequestedRestart();
	...
	public:
		...
		StringArray getOutputChannelNames() override {
			return internal->outStream != nullptr ? internal->outStream->chanNames : StringArray();
		}
		StringArray getInputChannelNames() override {
			return internal->inStream != nullptr ? internal->inStream->chanNames : StringArray();
		}
		...
		int getIndexOfDevice(bool asInput) const {
			if (auto dt = this->deviceType.lock()) {
				return dt->getDeviceNames(asInput).indexOf(getName());
			}
		...
		String open(const BigInteger &inputChannels,
			...
			if (bufferSizeSamples <= 0) bufferSizeSamples = getDefaultBufferSize();

			if (sampleRate <= 0) sampleRate = internal->getNominalSampleRate();
		...
		BigInteger getActiveOutputChannels() const override {
			return CoreAudioInternal::getActiveChannels(internal->outStream);
		}
		BigInteger getActiveInputChannels() const override {
			return CoreAudioInternal::getActiveChannels(internal->inStream);
		}
		...
		void start(AudioIODeviceCallback *callback) override {
			if (internal->start(callback)) previousCallback = callback;
		}
		...
		AudioIODeviceCallback *stopAndGetLastCallback() const {
			...
			if (lastCallback != nullptr) lastCallback->audioDeviceStopped();
		...
		AudioWorkgroup getWorkgroup() const override { return internal->audioWorkgroup; }

		bool isPlaying() override { return internal->isPlaying(); }

		String getLastError() override { return lastError; }

		void audioDeviceListChanged() {
			if (auto dv = deviceType.lock()) {
				dv->audioDeviceListChanged();
			}
			//			if (deviceType != nullptr) deviceType->audioDeviceListChanged();
		}
		...
		bool setCurrentSampleRate(double newSampleRate) { return internal->setNominalSampleRate(newSampleRate); }

		void setAsyncRestarter(AsyncRestarter *restarterIn) { restarter = restarterIn; }
	...
	private:
		...
		static OSStatus hardwareListenerProc(AudioDeviceID /*inDevice*/,
			...
			if (detailsChanged) static_cast<CoreAudioInternal *>(inClientData)->deviceDetailsChanged();
	...
	//==============================================================================
	class AudioIODeviceCombiner final
		: public AudioIODevice
	...
	public:
		AudioIODeviceCombiner(const String &deviceName,
			...
			: AudioIODevice(deviceName, "CoreAudio")
			, owner(deviceType->weak_from_this())
		...
		~AudioIODeviceCombiner() override { close(); }

		auto getDeviceWrappers() { return std::array<DeviceWrapper *, 2> {{&inputWrapper, &outputWrapper}}; }
		auto getDeviceWrappers() const {
			return std::array<const DeviceWrapper *, 2> {{&inputWrapper, &outputWrapper}};
		}

		int getIndexOfDevice(bool asInput) const {
			return asInput ? inputWrapper.getIndexOfDevice(true) : outputWrapper.getIndexOfDevice(false);
		}
		...
		int _jmin(int a, int b, int c) const noexcept { return std::min(a, std::min(b, c)); }

		int _jmax(int a, int b, int c) const noexcept { return std::max(a, std::max(b, c)); }
		int getCurrentBitDepth() override {
			return _jmin(32, inputWrapper.getCurrentBitDepth(), outputWrapper.getCurrentBitDepth());
		}

		int getDefaultBufferSize() override {
			return _jmax(0, inputWrapper.getDefaultBufferSize(), outputWrapper.getDefaultBufferSize());
		}

		AudioWorkgroup getWorkgroup() const override { return inputWrapper.getWorkgroup(); }
		...
		String open(const BigInteger &inputChannels,
			...
			if (bufferSize <= 0) bufferSize = getDefaultBufferSize();
		...
		void restart(AudioIODeviceCallback *cb) {
			...
			for (auto &d: getDeviceWrappers()) {
				...
				if (!approximatelyEqual(deviceSampleRate, sampleRateRequested)) {
					if (!getAvailableSampleRates().contains(deviceSampleRate)) return;

					for (auto &d2: getDeviceWrappers())
						if (&d2 != &d) d2->setCurrentSampleRate(deviceSampleRate);
			...
			for (auto &d: getDeviceWrappers()) {
				...
				if (deviceBufferSize != bufferSizeRequested) {
					if (!getAvailableBufferSizes().contains(deviceBufferSize)) return;
		...
		void restartAsync() override {
			{
				...
				if (active) {
					if (callback != nullptr) previousCallback = callback;
		...
		int getOutputLatencyInSamples() override { return targetLatency - getInputLatencyInSamples(); }

		int getInputLatencyInSamples() override { return inputWrapper.getLatencyInSamples(); }
		...
		void start(AudioIODeviceCallback *newCallback) override {
			...
			if (shouldStart) {
...

				{
					...
					else if (lastError.isEmpty()) lastError = "Failed to initialise all requested devices.";
				}
	...
	private:
		...
		void shutdown(const String &error) {
			...
			if (lastCallback != nullptr) {
				if (error.isNotEmpty()) lastCallback->audioDeviceError(error);
				else lastCallback->audioDeviceStopped();
			}
		}
		...
		template <typename Callback>
		void accessFifo(const uint64_t startPos,
						const int numChannels,
						const int numItems,
						Callback &&operateOnRange) {
			...
			for (int pos = 0; pos < numItems;) {
				...
				struct Args {
					int fifoPos, inputPos, nItems, channel;
				};
		...
		void inputAudioCallback(const float *const *channels,
								int numChannels,
								int n,
								const AudioIODeviceCallbackContext &context) noexcept {
			...
			if (currentWritePos == invalidSampleTime) return;
			...
			if (readPos != invalidSampleTime) {
				...
				if (readPos > currentWritePos
					|| (currentWritePos + static_cast<std::uint64_t>(n) - readPos)
						   > static_cast<std::uint64_t>(fifo.getNumSamples())) {
					xrun();
					return;
				}
			}
		...
		void outputAudioCallback(float *const *channels, int numChannels, int n) noexcept {
			...
			if (currentReadPos == invalidSampleTime) return;
			...
			const auto numZerosToWrite =
				sanitisedValidReadPos <= currentReadPos ? 0 : jmin(longN, sanitisedValidReadPos - currentReadPos);

			for (auto i = 0; i < numChannels; ++i)
				std::fill(channels[i], channels[i] + numZerosToWrite, 0.0f);

			accessFifo(currentReadPos + numZerosToWrite,
					   numChannels,
					   static_cast<int>(longN - numZerosToWrite),
					   [&](const auto &args) {
		...
		void handleAudioDeviceAboutToStart(AudioIODevice *device) {
			...
			if (anySampleRateChanges) {
				if (auto own = owner.lock()) {
					own->audioDeviceListChanged();
				}
			}
			if (callback != nullptr) callback->audioDeviceAboutToStart(device);
		}
		...
		struct DeviceWrapper final : public AudioIODeviceCallback {
			...
			~DeviceWrapper() override { device->close(); }

			void reset() { sampleTime.store(invalidSampleTime); }
			...
			void audioDeviceIOCallbackWithContext(const float *const *inputChannelData,
				...
				if (std::exchange(device->hadDiscontinuity, false)) owner.xrun();

				updateSampleTimeFromContext(context);

				if (input) owner.inputAudioCallback(inputChannelData, numInputChannels, numSamples, context);
				else owner.outputAudioCallback(outputChannelData, numOutputChannels, numSamples);
			}
			...
			StringArray getChannelNames() const {
				return input ? device->getInputChannelNames() : device->getOutputChannelNames();
			}
			BigInteger getActiveChannels() const {
				return input ? device->getActiveInputChannels() : device->getActiveOutputChannels();
			}
			int getLatencyInSamples() const {
				return input ? device->getInputLatencyInSamples() : device->getOutputLatencyInSamples();
			}
			...
			std::uint64_t nsToSampleTime(std::uint64_t ns) const noexcept {
				return static_cast<std::uint64_t>(
					std::round(static_cast<double>(ns) * device->getCurrentSampleRate() * 1e-9));
			}
			...
			void updateSampleTimeFromContext(const AudioIODeviceCallbackContext &context) noexcept {
				...
				if (input) callbackSampleTime += static_cast<std::uint64_t>(owner.targetLatency);
    ...
    */
		...
		public:
			ScopedErrorForwarder(AudioIODeviceCombiner &ownerIn, AudioIODeviceCallback *cb)
				...
				if (owner.callback == nullptr) owner.callback = this;
			}

			~ScopedErrorForwarder() override {
				const ScopedLock sl(owner.callbackLock);

				if (owner.callback == this) owner.callback = nullptr;
			}
			...
			void audioDeviceStopped() override {
				if (target != nullptr) target->audioDeviceStopped();
			...
			void audioDeviceError(const String &errorMessage) override {
				...
				if (target != nullptr) target->audioDeviceError(errorMessage);
	...
	//==============================================================================
	class CoreAudioIODeviceType final
		: public AudioIODeviceType
		, private AsyncUpdater
		, public std::enable_shared_from_this<CoreAudioIODeviceType> {
	public:
		...
		void scanForDevices() override {
			...
			auto audioDevices = audioObjectGetProperties<AudioDeviceID>(kAudioObjectSystemObject,
																		{kAudioHardwarePropertyDevices,
																		 kAudioObjectPropertyScopeWildcard,
																		 juceAudioObjectPropertyElementMain});

			for (const auto audioDevice: audioDevices) {
				if (const auto optionalName =
						audioObjectGetProperty<CFStringRef>(audioDevice,
															{kAudioDevicePropertyDeviceNameCFString,
															 kAudioObjectPropertyScopeWildcard,
															 juceAudioObjectPropertyElementMain})) {
					if (const CFUniquePtr<CFStringRef> name {*optionalName}) {
						const auto nameString = CFStringToASCIIStringIgnoringEmoji(name.get());
		...
		StringArray getDeviceNames(bool wantInputNames) const override {
			...
			return wantInputNames ? inputDeviceNames : outputDeviceNames;
		}
		...
		int getDefaultDeviceIndex(bool forInput) const override {
			...
			if (auto deviceID = audioObjectGetProperty<AudioDeviceID>(
					kAudioObjectSystemObject,
					{selector, kAudioObjectPropertyScopeWildcard, juceAudioObjectPropertyElementMain})) {
		...
		int getIndexOfDevice(AudioIODevice *device, bool asInput) const override {
			...
			if (auto *d = dynamic_cast<CoreAudioIODevice *>(device)) return d->getIndexOfDevice(asInput);

			if (auto *d = dynamic_cast<AudioIODeviceCombiner *>(device)) return d->getIndexOfDevice(asInput);
		...
		AudioIODevice *createDevice(const String &outputDeviceName, const String &inputDeviceName) override {
			...
			auto combinedName = outputDeviceName.isEmpty() ? inputDeviceName : outputDeviceName;

			if (inputDeviceID == outputDeviceID)
				return std::make_unique<CoreAudioIODevice>(this, combinedName, inputDeviceID, outputDeviceID)
					.release();

			auto in = inputDeviceID != 0
						  ? std::make_unique<CoreAudioIODevice>(this, inputDeviceName, inputDeviceID, 0)
						  : nullptr;

			auto out = outputDeviceID != 0
						   ? std::make_unique<CoreAudioIODevice>(this, outputDeviceName, 0, outputDeviceID)
						   : nullptr;
			...
			auto combo =
				std::make_unique<AudioIODeviceCombiner>(combinedName, this, std::move(in), std::move(out));
	...
	private:
		...
		void handleAsyncUpdate() override { audioDeviceListChanged(); }

		static int getNumChannels(AudioDeviceID deviceID, bool input) {
			int total = 0;

			if (auto bufList = audioObjectGetProperty<AudioBufferList>(deviceID,
																	   {kAudioDevicePropertyStreamConfiguration,
		...
		static OSStatus
			hardwareListenerProc(AudioDeviceID, UInt32, const AudioObjectPropertyAddress *, void *clientData) {
