...
*/
			...
			if (newNumSamples != size || newNumChannels != numChannels) {
				...
				if (keepExistingContent) {
					...
					if (avoidReallocating && allocatedBytes >= newTotalBytes) {
						if (clearExtraSpace || isClear) allocatedData.clear(newTotalBytes);
					} else {
						allocatedBytes = newTotalBytes;
    ...
    */
		void clear(int channel, int startSample, int numSamples) noexcept {
			...
			if (!isClear) FloatVectorOperations::clear(channels[channel] + startSample, numSamples);
		}
    ...
    */
		void applyGain(int channel, int startSample, int numSamples, Type gain) noexcept {
			...
			if (!approximatelyEqual(gain, Type(1)) && !isClear) {
				...
				if (approximatelyEqual(gain, Type())) FloatVectorOperations::clear(d, numSamples);
				else FloatVectorOperations::multiply(d, gain, numSamples);
			}
		}
    ...
    */
		...
		void applyGain(Type gain) noexcept { applyGain(0, size, gain); }
		...
		/*void addFrom(int destChannel,
			...
			if (!approximatelyEqual(gainToApplyToSource, (Type) 0) && numSamples > 0 && !source.isClear) {
				...
				if (isClear) {
					...
					else FloatVectorOperations::copy(d, s, numSamples);
				} else {
					if (!approximatelyEqual(gainToApplyToSource, Type(1)))
						FloatVectorOperations::addWithMultiply(d, s, gainToApplyToSource, numSamples);
					else FloatVectorOperations::add(d, s, numSamples);
				}
		...
		void addFrom(int destChannel,
			...
			if (!approximatelyEqual(gainToApplyToSource, Type()) && numSamples > 0) {
				...
				if (isClear) {
					...
					else FloatVectorOperations::copy(d, source, numSamples);
				} else {
					if (!approximatelyEqual(gainToApplyToSource, Type(1)))
						FloatVectorOperations::addWithMultiply(d, source, gainToApplyToSource, numSamples);
					else FloatVectorOperations::add(d, source, numSamples);
				}
			}
		}
*/
			...
			if (numSamples > 0) {
				...
				if (!approximatelyEqual(gain, Type(1))) {
					if (approximatelyEqual(gain, Type())) {
						if (!isClear) FloatVectorOperations::clear(d, numSamples);
					} else {
						isClear = false;
...
*/
		...
		Type getRMSLevel(int channel, int startSample, int numSamples) const noexcept {
			...
			if (numSamples <= 0 || channel < 0 || channel >= numChannels || isClear) return Type(0);
	...
	private:
		...
		void allocateData() {
			...
			if (alignmentOverflow != 0) channelListSize += requiredSampleAlignment - alignmentOverflow;
    ...
    */
	...
	bool operator==(const AudioBuffer<Type> &a, const AudioBuffer<Type> &b) {
		...
		for (auto c = 0; c < a.getNumChannels(); ++c) {
			...
			if (!std::equal(begin(a), end(a), begin(b), end(b))) return false;
		}
