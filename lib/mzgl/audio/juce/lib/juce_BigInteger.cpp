...
namespace juce {
	...
	namespace {
		inline uint32 bitToMask(const int bit) noexcept {
			return (uint32) 1 << (bit & 31);
		}
		inline size_t bitToIndex(const int bit) noexcept {
			return (size_t) (bit >> 5);
		}
		inline size_t sizeNeededToHold(int highestBit) noexcept {
	...
	BigInteger::BigInteger(const BigInteger &other)
		...
		if (allocatedSize > numPreallocatedInts) heapAllocation.malloc(allocatedSize);
	...
	BigInteger &BigInteger::operator=(const BigInteger &other) {
		if (this != &other) {
			...
			if (newAllocatedSize <= numPreallocatedInts) heapAllocation.free();
			else if (newAllocatedSize != allocatedSize) heapAllocation.malloc(newAllocatedSize);
	...
	uint32 *BigInteger::getValues() const noexcept {
		...
		return heapAllocation != nullptr ? heapAllocation.get() : const_cast<uint32 *>(preallocated);
	}
	...
	uint32 *BigInteger::ensureSize(const size_t numVals) {
		if (numVals > allocatedSize) {
			...
			if (heapAllocation == nullptr) {
				...
				memcpy(heapAllocation.get(), preallocated, sizeof(uint32) * numPreallocatedInts);
			} else {
	...
	bool BigInteger::operator[](const int bit) const noexcept {
		return bit <= highestBit && bit >= 0 && ((getValues()[bitToIndex(bit)] & bitToMask(bit)) != 0);
	}
	...
	BigInteger &BigInteger::clearBit(const int bit) noexcept {
		if (bit >= 0 && bit <= highestBit) {
			...
			if (bit == highestBit) highestBit = getHighestBit();
		}
...
#	pragma intrinsic(_BitScanReverse)
	...
	int BigInteger::getHighestBit() const noexcept {
		...
		for (int i = (int) bitToIndex(highestBit); i >= 0; --i)
			if (uint32 n = values[i]) return findHighestSetBit(n) + (i << 5);
	...
	int BigInteger::findNextSetBit(int i) const noexcept {
		...
		for (; i <= highestBit; ++i)
			if ((values[bitToIndex(i)] & bitToMask(i)) != 0) return i;
	...
	int BigInteger::findNextClearBit(int i) const noexcept {
		...
		for (; i <= highestBit; ++i)
			if ((values[bitToIndex(i)] & bitToMask(i)) == 0) break;
	...
	//==============================================================================
	BigInteger &BigInteger::operator+=(const BigInteger &other) {
		if (this == &other) return operator+=(BigInteger(other));

		if (other.isNegative()) return operator-=(-other);
		...
		if (isNegative()) {
			...
			for (size_t i = 0; i < numInts; ++i) {
				...
				if (i < other.allocatedSize) remainder += otherValues[i];
	...
	BigInteger &BigInteger::operator-=(const BigInteger &other) {
		...
		for (size_t i = 0; i < numInts; ++i) {
			if (i < maxOtherInts) amountToSubtract += (int64) otherValues[i];
	...
	void BigInteger::divideBy(const BigInteger &divisor, BigInteger &remainder) {
		if (this == &divisor) return divideBy(BigInteger(divisor), remainder);
	...
	BigInteger &BigInteger::operator|=(const BigInteger &other) {
		...
		if (other.highestBit >= 0) {
			...
			if (other.highestBit > highestBit) highestBit = other.highestBit;
	...
	int BigInteger::compareAbsolute(const BigInteger &other) const noexcept {
		...
		for (int i = (int) bitToIndex(h1); i >= 0; --i)
			if (values[i] != otherValues[i]) return values[i] > otherValues[i] ? 1 : -1;
	...
	BigInteger &BigInteger::shiftBits(int bits, const int startBit) {
		if (highestBit >= 0) {
			if (bits < 0) shiftRight(-bits, startBit);
			else if (bits > 0) shiftLeft(bits, startBit);
		}
	...
	static BigInteger simpleGCD(BigInteger *m, BigInteger *n) {
		while (!m->isZero()) {
			if (n->compareAbsolute(*m) > 0) std::swap(m, n);
	...
	BigInteger BigInteger::findGreatestCommonDivisor(BigInteger n) const {
		...
		while (!n.isZero()) {
			if (std::abs(m.getHighestBit() - n.getHighestBit()) <= 16) return simpleGCD(&m, &n);
		...
		if (modulus.getHighestBit() <= 32 || modulus % 2 == 0) {
			...
			for (int i = n; --i >= 0;) {
				*this *= *this;
				...
				if (compareAbsolute(modulus) >= 0) *this %= modulus;
			}
		} else {
			...
			if (!g.isOne()) {
				...
				for (int i = exp.getHighestBit(); --i >= 0;) {
					...
					if (exp[i]) xm.montgomeryMultiplication(am, modulus, m1, Rfactor);
				}
	...
	void BigInteger::montgomeryMultiplication(const BigInteger &other,
		...
		*this += t;
		...
		if (compare(modulus) >= 0) *this -= modulus;
		else if (isNegative()) *this += modulus;
	}

	void BigInteger::extendedEuclidean(const BigInteger &a, const BigInteger &b, BigInteger &x, BigInteger &y) {
	...
	void BigInteger::inverseModulo(const BigInteger &modulus) {
		...
		if (isNegative() || compareAbsolute(modulus) >= 0) *this %= modulus;
		...
		BigInteger a1(modulus), a2(*this), b1(modulus), b2(1);
	...
	//
	//				if (remainder.isZero() && v.isZero()) break;
	//
	...
	//			values[i] = (uint32) ByteOrder::littleEndianInt(
	//				addBytesToPointer(data.getData(), (size_t) i * sizeof(uint32)));
	//
	//		values[numInts - 1] = 0;
	//
	...
	void writeLittleEndianBitsInBuffer(void *buffer, uint32 startBit, uint32 numBits, uint32 value) noexcept {
		...
		if (numBits > 0) *data = (uint8) ((*data & (uint32) (0xff << numBits)) | value);
	}
	...
	uint32 readLittleEndianBitsInBuffer(const void *buffer, uint32 startBit, uint32 numBits) noexcept {
		...
		if (const uint32 offset = (startBit & 7)) {
			...
			if (bitsInByte >= numBits) return result & ((1u << numBits) - 1u);
		...
		if (numBits > 0) result |= ((*data & ((1u << numBits) - 1u)) << bitsRead);
