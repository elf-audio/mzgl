...
bool AudioFileAndroid_load(std::string path, FloatBuffer &buff, int *outNumChannels, int *outSampleRate, int desiredSampleRate = 0) {
        ...
        if (isDecoding){
            ...
            if (outputIndex >= 0){
                ...
                if (info.flags & AMEDIACODEC_BUFFER_FLAG_END_OF_STREAM){
                    ...
                    for(int i = 0; i < numSamples; i++) fBuff[i] = (float)outBuff[i]*sint16ToFloat;
