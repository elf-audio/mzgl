...
public:
	...
	virtual std::vector<std::string> getFactoryPresetNames() { return factoryPresets; }
	...
	size_t getNumPresets() { return factoryPresets.size(); }
...
public:
	...
	bool getHostIsPlaying() { return hostIsPlaying; }
...
protected:
	...
	void addFloatParameter(
		std::string name, float defaultValue, float from = 0, float to = 1, std::string unit = "") {
