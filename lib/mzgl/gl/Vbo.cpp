...
void Vbo::printVbos() {
	...
	for(int i = 0; i < vbos.size(); i++) {
		...
		Log::d() << vbos[i]->vertexArrayObject << ", " << vbos[i]->vertexBuffer<<", "<<vbos[i]->colorbuffer<<", " <<vbos[i]->texCoordBuffer<<", " << vbos[i]->normalBuffer<< ", " << vbos[i]->indexBuffer;
	}
    Log::e() <<"END -----------------------------------------";
}
#endif
...
//        Log::e() << "Vertex is 0";
	...
	glVertexAttribPointer(
						g.currShader->positionAttribute,
						  vertDimensions,     // size
						  GL_FLOAT,           // type
						  GL_FALSE,           // normalized?
						  0,                  // stride
	...
	if(colorbuffer!=0) {
		if(g.currShader->colorAttribute==-1) {
			Log::e() << "There is no Color attribute in this shader";
		} else {
	...
	if(texCoordBuffer!=0) {
		...
		if(g.currShader->texCoordAttribute==-1) {
			...
			glVertexAttribPointer(
								  g.currShader->texCoordAttribute,
								  2,                                // size
								  GL_FLOAT,                         // type
								  GL_FALSE,                         // normalized?
								  0,                                // stride
