...
ScopedAlphaBlend::ScopedAlphaBlend(Graphics &g, bool shouldBlend) : g(g) {
...
void Graphics::setupView(bool flipped, int w, int h) {
	...
	viewMatrix       = glm::lookAt(
											 glm::vec3(eyeX, eyeY, dist*(flipped?-1:1)), // Camera is at (4,3,-3), in World Space
...
void Graphics::warpMaskForScissor(Rectf &a) {
	// if we're on linux, not android, and pixelScale==2, then offset the y by g.height/2, and divide all rect dimensions by 2

#if defined(__linux__) && !defined(__ANDROID__)
    a.x /= pixelScale;
    a.y /= pixelScale;
    a.y += height/pixelScale;
