...
/*
    ...
    - provide a unified shaders language, instead 3D-API-specific shaders
      source-code or shaders-bytecode must be provided (for the "official"
      offline shaders cross-compiler, see here:
    ...
    --- fill an sg_bindings struct with the resource bindings for the next
        ...
        0..N sampler objects on the vertex-shaders- and fragment-shaders-stage
        and then call
    ...
    --- optionally update shaders uniform data with:
    ...
    ON SHADER CREATION
    ==================
    sokol-gfx doesn't come with an integrated shaders cross-compiler, instead
    backend-specific shaders sources or binary blobs need to be provided when
    creating a shaders object, along with information about the shaders resource
    binding interface needed in the sokol-gfx validation layer and to properly
    bind shaders resources on the CPU-side to be consumable by the GPU-side.

    The easiest way to provide all this shaders creation data is to use the
    sokol-shdc shaders compiler tool to compile shaders from a common
    GLSL syntax into backend-specific sources or binary blobs, along with
    shaders interface information and uniform blocks mapped to C structs.

    To create a shaders using a C header which has been code-generated by sokol-shdc:

        // include the C header code-generated by sokol-shdc:
        #include "myshader.glsl.h"
        ...

        // create shaders using a code-generated helper function from the C header:
    ...
    To create shaders with backend-specific shaders code or binary blobs,
    the sg_make_shader() function requires the following information:

    - Shader code or shaders binary blobs for the vertex- and fragment- shaders-stage:
        ...
        - for the D3D11 backend, shaders can be provided as source or binary blobs, the
          ...
          shaders source code is provided for the D3D11 backend, sokol-gfx will dynamically
        ...
        - optionally the following shaders-code related attributes can be provided:
    ...
    - Depending on backend, information about the input vertex attributes used by the
      vertex shaders:
        - Metal: no information needed since vertex attributes are always bound
          by their attribute location defined in the shaders via '[[attribute(N)]]'
        - WebGPU: no information needed since vertex attributes are always
          bound by their attribute location defined in the shaders via `@location(N)`
        ...
        - D3D11: a 'semantic name' and 'semantic index' must be provided for each vertex
          attribute, e.g. if the vertex attribute is defined as 'TEXCOORD1' in the shaders,
          the semantic name would be 'TEXCOORD', and the semantic index would be '1'

    - Information about each uniform block used in the shaders:
    ...
    - A description of each texture/image used in the shaders:
    ...
    - A description of each sampler used in the shaders:
        - SG_SAMPLERTYPE_FILTERING,
        - SG_SAMPLERTYPE_NONFILTERING,
        - SG_SAMPLERTYPE_COMPARISON,

    - An array of 'image-sampler-pairs' used by the shaders to sample textures,
    ...
    For example code of how to create backend-specific shaders objects,
    ...
    The information of whether a shaders is going to be used with such an
    unfilterable-float texture must already be provided in the sg_shader_desc
    struct when creating the shaders (see the above section "ON SHADER CREATION").

    If you are using the sokol-shdc shaders compiler, the information whether a
    texture/sampler binding expects an 'unfilterable-float/nonfiltering'
    texture/sampler combination cannot be inferred from the shaders source
    alone, you'll need to provide this hint via annotation-tags. For instance
    here is an example from the ozz-skin-sapp.c sample shaders which samples an
    RGBA32F texture with skinning matrices in the vertex shaders:
    ...
    UNIFORM DATA LAYOUT:
    ====================
    NOTE: if you use the sokol-shdc shaders compiler tool, you don't need to worry
    about the following details.

    The data that's passed into the sg_apply_uniforms() function must adhere to
    specific layout rules so that the GPU shaders finds the uniform block
    ...
    For example if the GLSL shaders has the following uniform declarations:
    ...
    to use the sokol-shdc shaders cross-compiler tool!
    ...
    - D3D11 can only convert *normalized* vertex formats to
      ...
      vertex shaders inputs, those can only be uses with the *ivecn* vertex shaders
    ...
    vertex formats, and if needed "expand" the normalized vertex shaders
    inputs in the vertex shaders by multiplying with 127.0, 255.0, 32767.0 or
    65535.0:
    ...
    to create a pipeline object with a shaders that's not in VALID state will
    ...
    - when writing WGSL shaders code by hand, a specific bind-slot convention
      must be used:

      All uniform block structs must use `@group(0)`, with up to
      4 uniform blocks per shaders stage.
        - Vertex shaders uniform block bindings must start at `@group(0) @binding(0)`
        - Fragment shaders uniform blocks bindings must start at `@group(0) @binding(4)`

      All textures and samplers must use `@group(1)` and start at specific
      offsets depending on resource type and shaders stage.
        - Vertex shaders textures must start at `@group(1) @binding(0)`
        - Vertex shaders samplers must start at `@group(1) @binding(16)`
        - Fragment shaders textures must start at `@group(1) @binding(32)`
        - Fragment shaders samplers must start at `@group(1) @binding(48)`
      ...
      12 textures and 8 samplers per shaders stage are allowed).

      If you use sokol-shdc to generate WGSL shaders code, you don't need to worry
    ...
    - The sokol shaders compiler generally adds `diagnostic(off, derivative_uniformity);`
...
/*
    ...
    sg_sampler      sampler object describing how a texture is sampled in a shaders
    sg_shader:      vertex- and fragment-shaders and shaders interface information
    sg_pipeline:    associated shaders and vertex-layouts, and render states
...
/*
    ...
    in the shaders (both must match and will be checked in the validation layer
...
/*
    ...
    The basic data type of a texture sample as expected by a shaders.
    ...
    is compatible with what the shaders expects. Apart from the sokol-gfx
...
/*
    ...
    defined in a shaders. Must be provided in sg_shader_sampler_desc.
...
/*
    ...
    There are 2 shaders stages: vertex- and fragment-shaders-stage.
    Each shaders stage consists of:

    - one slot for a shaders function (provided as source- or byte-code)
    - SG_MAX_SHADERSTAGE_UBS slots for uniform blocks
    - SG_MAX_SHADERSTAGE_IMAGES slots for images used as textures by
      the shaders function
*/
...
/*
    ...
    - 0..N vertex shaders stage images
    - 0..N vertex shaders stage samplers
    - 0..N fragment shaders stage images
    - 0..N fragment shaders stage samplers

    The max number of vertex buffer and shaders stage images
...
/*
    ...
    shaders-resource-view / texture-view object. If you want to use access the
    injected texture in a shaders you *must* provide a shaders-resource-view.
...
/*
    ...
    The structure sg_shader_desc defines all creation parameters for shaders
    programs, used as input to the sg_make_shader() function:

    - reflection information for vertex attributes (vertex shaders inputs):
        - vertex attribute name (only optionally used by GLES3 and GL)
        - a semantic name and index (required for D3D11)
    - for each shaders-stage (vertex and fragment):
        - the shaders source or bytecode
        ...
        - reflection info for each uniform block used by the shaders stage:
        ...
        - reflection info for textures used in the shaders stage:
            - the image type (SG_IMAGETYPE_xxx)
            - the image-sample type (SG_IMAGESAMPLETYPE_xxx, default is SG_IMAGESAMPLETYPE_FLOAT)
            - whether the shaders expects a multisampled texture
        - reflection info for samplers used in the shaders stage:
            - the sampler type (SG_SAMPLERTYPE_xxx)
        - reflection info for each image-sampler-pair used by the shaders:
    ...
    For all GL backends, shaders source-code must be provided. For D3D11 and Metal,
    either shaders source-code or byte-code can be provided.

    For D3D11, if source code is provided, the d3dcompiler_47.dll will be loaded
    on demand. If this fails, shaders creation will fail. When compiling HLSL
    source code, you can provide an optional target string via
    sg_shader_stage_desc.d3d11_target, the default target is "vs_4_0" for the
    vertex shaders stage and "ps_4_0" for the pixel shaders stage.
*/
...
/*
    ...
    .shaders:            0 (must be initialized with a valid sg_shader id!)
    .layout:
...
#define _SG_LOG_ITEMS \
    ...
    _SG_LOGITEM_XMACRO(GL_SHADER_COMPILATION_FAILED, "shaders compilation failed (gl)") \
    _SG_LOGITEM_XMACRO(GL_SHADER_LINKING_FAILED, "shaders linking failed (gl)") \
    _SG_LOGITEM_XMACRO(GL_VERTEX_ATTRIBUTE_NOT_FOUND_IN_SHADER, "vertex attribute not found in shaders (gl)") \
    _SG_LOGITEM_XMACRO(GL_TEXTURE_NAME_NOT_FOUND_IN_SHADER, "texture name not found in shaders (gl)") \
    ...
    _SG_LOGITEM_XMACRO(D3D11_SHADER_COMPILATION_FAILED, "shaders compilation failed (d3d11)") \
    ...
    _SG_LOGITEM_XMACRO(METAL_SHADER_COMPILATION_FAILED, "shaders compilation failed (metal)") \
    _SG_LOGITEM_XMACRO(METAL_SHADER_CREATION_FAILED, "shaders creation failed (metal)") \
    _SG_LOGITEM_XMACRO(METAL_SHADER_COMPILATION_OUTPUT, "") \
    _SG_LOGITEM_XMACRO(METAL_VERTEX_SHADER_ENTRY_NOT_FOUND, "vertex shaders entry function not found (metal)") \
    _SG_LOGITEM_XMACRO(METAL_FRAGMENT_SHADER_ENTRY_NOT_FOUND, "fragment shaders entry not found (metal)") \
    ...
    _SG_LOGITEM_XMACRO(WGPU_SHADER_TOO_MANY_IMAGES, "shaders uses too many sampled images on shaders stage (wgpu)") \
    _SG_LOGITEM_XMACRO(WGPU_SHADER_TOO_MANY_SAMPLERS, "shaders uses too many samplers on shaders stage (wgpu)") \
    _SG_LOGITEM_XMACRO(WGPU_SHADER_CREATE_BINDGROUP_LAYOUT_FAILED, "wgpuDeviceCreateBindGroupLayout() for shaders stage failed") \
    ...
    _SG_LOGITEM_XMACRO(DEALLOC_SHADER_INVALID_STATE, "sg_dealloc_shader(): shaders must be in ALLOC state") \
    ...
    _SG_LOGITEM_XMACRO(INIT_SHADER_INVALID_STATE, "sg_init_shader(): shaders must be in ALLOC state") \
    ...
    _SG_LOGITEM_XMACRO(UNINIT_SHADER_INVALID_STATE, "sg_uninit_shader(): shaders must be in VALID or FAILED state") \
    ...
    _SG_LOGITEM_XMACRO(FAIL_SHADER_INVALID_STATE, "sg_fail_shader(): shaders must be in ALLOC state") \
    ...
    _SG_LOGITEM_XMACRO(SHADER_POOL_EXHAUSTED, "shaders pool exhausted") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_SOURCE, "shaders source code required") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_BYTECODE, "shaders byte code required") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_SOURCE_OR_BYTECODE, "shaders source or byte code required") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_BYTECODE_SIZE, "shaders byte code length (in bytes) required") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_UBS, "shaders uniform blocks must occupy continuous slots") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_IMAGES, "shaders stage images must occupy continuous slots (sg_shader_desc.vs|fs.images[])") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_SAMPLERS, "shaders stage samplers must occupy continuous slots (sg_shader_desc.vs|fs.samplers[])") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_IMAGE_SLOT_OUT_OF_RANGE, "shaders stage: image-sampler-pair image slot index is out of range (sg_shader_desc.vs|fs.image_sampler_pairs[].image_slot)") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_SAMPLER_SLOT_OUT_OF_RANGE, "shaders stage: image-sampler-pair image slot index is out of range (sg_shader_desc.vs|fs.image_sampler_pairs[].sampler_slot)") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_NAME_REQUIRED_FOR_GL, "shaders stage: image-sampler-pairs must be named in GL (sg_shader_desc.vs|fs.image_sampler_pairs[].name)") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_HAS_NAME_BUT_NOT_USED, "shaders stage: image-sampler-pair has name but .used field not true") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_HAS_IMAGE_BUT_NOT_USED, "shaders stage: image-sampler-pair has .image_slot != 0 but .used field not true") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_SAMPLER_PAIR_HAS_SAMPLER_BUT_NOT_USED, "shaders stage: image-sampler-pair .sampler_slot != 0 but .used field not true") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NONFILTERING_SAMPLER_REQUIRED, "shaders stage: image sample type UNFILTERABLE_FLOAT, UINT, SINT can only be used with NONFILTERING sampler") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_COMPARISON_SAMPLER_REQUIRED, "shaders stage: image sample type DEPTH can only be used with COMPARISON sampler") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_IMAGE_NOT_REFERENCED_BY_IMAGE_SAMPLER_PAIRS, "shaders stage: one or more images are note referenced by  (sg_shader_desc.vs|fs.image_sampler_pairs[].image_slot)") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_SAMPLER_NOT_REFERENCED_BY_IMAGE_SAMPLER_PAIRS, "shaders stage: one or more samplers are not referenced by image-sampler-pairs (sg_shader_desc.vs|fs.image_sampler_pairs[].sampler_slot)") \
    _SG_LOGITEM_XMACRO(VALIDATE_SHADERDESC_NO_CONT_IMAGE_SAMPLER_PAIRS, "shaders stage image-sampler-pairs must occupy continuous slots (sg_shader_desc.vs|fs.image_samplers[])") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_SHADER, "sg_pipeline_desc.shaders missing or invalid") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_PIPELINEDESC_ATTR_SEMANTICS, "D3D11 missing vertex attribute semantics in shaders") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_APIP_SHADER_EXISTS, "sg_apply_pipeline: shaders object no longer alive") \
    _SG_LOGITEM_XMACRO(VALIDATE_APIP_SHADER_VALID, "sg_apply_pipeline: shaders object not in valid state") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_IMAGE_TYPE_MISMATCH, "sg_apply_bindings: type of image bound to vertex stage doesn't match shaders desc") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_UNEXPECTED_SAMPLER_COMPARE_NEVER, "sg_apply_bindings: shaders expects SG_SAMPLERTYPE_COMPARISON on vertex stage but sampler has SG_COMPAREFUNC_NEVER") \
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_EXPECTED_SAMPLER_COMPARE_NEVER, "sg_apply_bindings: shaders expects SG_SAMPLERTYPE_FILTERING or SG_SAMPLERTYPE_NONFILTERING on vertex stage but sampler doesn't have SG_COMPAREFUNC_NEVER") \
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_VS_EXPECTED_NONFILTERING_SAMPLER, "sg_apply_bindings: shaders expected SG_SAMPLERTYPE_NONFILTERING on vertex stage, but sampler has SG_FILTER_LINEAR filters") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_IMAGE_TYPE_MISMATCH, "sg_apply_bindings: type of image bound to fragment stage doesn't match shaders desc") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_UNEXPECTED_SAMPLER_COMPARE_NEVER, "sg_apply_bindings: shaders expects SG_SAMPLERTYPE_COMPARISON on fragment stage but sampler has SG_COMPAREFUNC_NEVER") \
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_EXPECTED_SAMPLER_COMPARE_NEVER, "sg_apply_bindings: shaders expects SG_SAMPLERTYPE_FILTERING on fragment stage but sampler doesn't have SG_COMPAREFUNC_NEVER") \
    _SG_LOGITEM_XMACRO(VALIDATE_ABND_FS_EXPECTED_NONFILTERING_SAMPLER, "sg_apply_bindings: shaders expected SG_SAMPLERTYPE_NONFILTERING on fragment stage, but sampler has SG_FILTER_LINEAR filters") \
    ...
    _SG_LOGITEM_XMACRO(VALIDATE_AUB_NO_UB_AT_SLOT, "sg_apply_uniforms: no uniform block declaration at this shaders stage UB slot") \
...
// D3D11: get internal shaders resource objects
...
// Metal: get internal __bridge-casted shaders resource objects
...
// WebGPU: get internal shaders resource objects
...
// GL: get internal shaders resource objects
...
#define _SG_WGPU_NUM_BINDGROUPS (2) // 0: uniforms, 1: images and sampler on both shaders stages
...
_SOKOL_PRIVATE GLuint _sg_gl_compile_shader(sg_shader_stage stage, const char* src) {
    ...
    if (!compile_status) {
        // compilation failed, log error and delete shaders
        GLint log_len = 0;
...
_SOKOL_PRIVATE sg_resource_state _sg_d3d11_create_image(_sg_image_t* img, const sg_image_desc* desc) {
    ...
    if (img->cmn.type != SG_IMAGETYPE_3D) {
        ...
        if (injected) {
            ...
            // create shaders-resource-view for 2D texture
            // FIXME: currently we don't support setting MSAA texture as shaders resource
            if (!msaa) {
        ...
        if (injected) {
            ...
            // create shaders-resource-view for 3D texture
            if (!msaa) {
                D3D11_SHADER_RESOURCE_VIEW_DESC d3d11_srv_desc;
                _sg_clear(&d3d11_srv_desc, sizeof(d3d11_srv_desc));
                d3d11_srv_desc.Format = _sg_d3d11_srv_pixel_format(img->cmn.pixel_format);
...
_SOKOL_PRIVATE sg_resource_state _sg_d3d11_create_shader(_sg_shader_t* shd, const sg_shader_desc* desc) {
    ...
    // shaders stage uniform blocks and image slots
    for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    ...
    if (vs_ptr && fs_ptr && (vs_length > 0) && (fs_length > 0)) {
        // create the D3D vertex- and pixel-shaders objects
        ...
        // need to store the vertex shaders byte code, this is needed later in sg_create_pipeline
...
_SOKOL_PRIVATE bool _sg_mtl_apply_bindings(_sg_bindings_t* bnd) {
    ...
    // apply fragment shaders samplers
...
_SOKOL_PRIVATE void _sg_wgpu_setup_backend(const sg_desc* desc) {
    ...
    // create an empty bind group for shaders stages without bound images
...
_SOKOL_PRIVATE bool _sg_validate_shader_desc(const sg_shader_desc* desc) {
        ...
        // if shaders byte code, the size must also be provided
...
_SOKOL_PRIVATE bool _sg_validate_apply_pipeline(sg_pipeline pip_id) {
        ...
        // the pipeline's shaders must be alive and valid
...
_SOKOL_PRIVATE bool _sg_validate_apply_bindings(const sg_bindings* bindings) {
        ...
        // has expected vertex shaders images
        ...
        // has expected vertex shaders image samplers
        ...
        // has expected fragment shaders images
        for (int i = 0; i < SG_MAX_SHADERSTAGE_IMAGES; i++) {
        ...
        // has expected fragment shaders samplers
        for (int i = 0; i < SG_MAX_SHADERSTAGE_SAMPLERS; i++) {
            const _sg_shader_stage_t* stage = &pip->shader->cmn.stage[SG_SHADERSTAGE_FS];
            if (stage->samplers[i].sampler_type != _SG_SAMPLERTYPE_DEFAULT) {
