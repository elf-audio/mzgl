...
// DRAW A TRIANGLE
        ...
        // this draws 3 turquoise triangles on top of eachother
        // and rotates them

        // set colour to turquoise with alpha of 33%
        g.setColor(0, 1, 1, 0.33);
...
// DRAW A polygon
        ...
        for(float angle = 0.f; angle <= M_PI * 2.02; angle += M_PI / 64.f) {
            float radius = 100 + std::sin(angle*8.f) * 30;
        ...
        g.drawVerts(verts, GL_TRIANGLE_FAN);
        g.popMatrix();
    }
};
